...
% strategia 2 con cambio forma pericentro pericentro 
...
%scegliamo come punto di manovra il secondo perchè graficamente abbiamo
%visto che conviene per il tempo
theta_1=vect_theta_1(2);
deltati1=vect_deltati1(2);
thetaorb1=vect_thetaorb1(2);

%% cambio forma pericentro pericentro
w_2=w_1+pi;

[deltav2,deltav3,deltat12,theta_2,ah,eh] = changeShape (ai,ei,w_1,a_f,e_f,w_2,theta_1,0);

hold on

plot_orbit(ai,ei,ii,omegai,w_1,theta_1,theta_1,2*pi)
plot_orbit(ah,eh,ii,omegai,w_1,0,0,pi)

%% cambio inclinazione

[deltav4,w_3,theta_3] = changeOrbitalPlane(a_f,e_f,ii,omegai,w_2,i_f,omega_f);
theta_3=2*pi+theta_3;  %in uscita theta_3 è negativo, lo rendiamo quindi positivo
[deltat23] = timeOfFlight(a_f,e_f,theta_2,theta_3);

plot_orbit(a_f,e_f,ii,omegai,w_2,0,0,theta_3)

%% secondo cambio di anomalia di pericentro

[deltav5,w_4,vect_theta_4,vect_deltat34,vect_thetaorb4] = changePeriapsisArg(a_f,e_f,w_3,theta_3,w_f);
theta_4=vect_theta_4(1)-2*pi;
